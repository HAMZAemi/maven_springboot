   ...
   stages {
      ...
      stage('Deploy Docker Container on Server') {
         steps {
            ...
            sh "sshpass -p ${SERVER_PASSWORD} ssh ${SERVER_USERNAME}@${SERVER_IP} 'echo ${HARBOR_PASSWORD} | docker login --username ${HARBOR_USERNAME} --password-stdin --insecure-skip-tls-verify ${DOCKER_REGISTRY}'"
